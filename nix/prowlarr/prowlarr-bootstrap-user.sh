...
prowlarr_config_xml="/appdata/config.xml"
...
. /run/prowlarr-bootstrap.env

username="${PROWLARR_BOOTSTRAP_USERNAME:-}"
password="${PROWLARR_BOOTSTRAP_PASSWORD:-}"
...
systemctl restart prowlarr.service

wait_api_ready || { log "Prowlarr API did not become ready after restart"; exit 1; }
