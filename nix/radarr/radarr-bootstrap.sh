...
synchronize_api_key() {
  ...
  if [ "$radarr_api_key" = "$desired_api_key" ]; then
    return 0
  fi

  escaped="$(printf '%s' "$desired_api_key" | sed 's/[&|]/\\&/g')"
  if ! grep -q "<ApiKey>" "$config_xml"; then
    log "Cannot synchronize Radarr API key: <ApiKey> tag missing in $config_xml"
    return 1
  fi

  sed -i "s|<ApiKey>[^<]*</ApiKey>|<ApiKey>${escaped}</ApiKey>|g" "$config_xml"
  systemctl restart radarr.service
  radarr_api_key="$desired_api_key"
  wait_api_ready || { log "Radarr API did not become ready after API key synchronization"; return 1; }
  log "Synchronized Radarr API key with shared secret"
}
